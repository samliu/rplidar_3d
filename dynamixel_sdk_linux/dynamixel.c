...
void dxl_terminate() { dxl_hal_close(); }
...
int dxl_get_result() { return gbCommStatus; }
...
int dxl_get_rxpacket_length() { return (int)gbStatusPacket[LENGTH]; }
...
void dxl_sync_write_start(int address, int data_length) {
  while (giBusUsing)
    ; // needs to be done before touching the TX buffer as it is used until the
      // end of RX.

  gbInstructionPacket[ID] =
      BROADCAST_ID; // use the device ID of the USB2AX instead of the broadcast
                    // ID to avoid some modifications to the RX code.
  gbInstructionPacket[INSTRUCTION] = INST_SYNC_WRITE;
  gbInstructionPacket[PARAMETER] = (unsigned char)address;
...
void dxl_sync_write_push_word(int value) {
  ...
  gbInstructionPacket[PARAMETER + gbSyncNbParam++] =
      (unsigned char)dxl_get_lowbyte(value);
  gbInstructionPacket[PARAMETER + gbSyncNbParam++] =
      (unsigned char)dxl_get_highbyte(value);
}
...
void dxl_sync_read_start(int address, int data_length) {
  ...
  gbInstructionPacket[ID] = 0XFD; // use the device ID of the USB2AX instead of
                                  // the broadcast ID to avoid some
                                  // modifications to the rx code.
  gbInstructionPacket[INSTRUCTION] = INST_SYNC_READ;
  gbInstructionPacket[PARAMETER] = (unsigned char)address;
  gbInstructionPacket[PARAMETER + 1] = (unsigned char)data_length;
  gbSyncNbParam = 2;
